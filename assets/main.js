...
(function(){
  ...
  function makeCard(it){
    ...
    // Par défaut, pour la catégorie "algorithme" on ouvre le viewer.
    if (category === 'algorithme') {
      openHref = 'viewer.html?title=' + encodeURIComponent(it.title || filename);
    }

    // Si l'entry définit explicitement "liens", on le préfère (ouvre directement ce .html)
    // Exemples valides dans downloads.js: "liens": "path/to/page.html" ou "/static/nom.html" ou full URL.
    const liens = (it.liens || '').toString().trim();
    if (liens.length) {
      openHref = liens;
    }
